// ================================
// iDev Meet - app.js (v3 - CORRIGIDO ordem register-peer)
// ================================
...
// ================================
// ✅ PeerJS com TURN público (OpenRelay)
// TURN é ESSENCIAL para usuários em redes diferentes (NAT/AWS)
// ================================
const peer = new Peer(undefined, {
  ...
  config: {
    iceServers: [
      ...
      // ✅ TURN público gratuito — permite conexões entre redes NAT diferentes
      {
...
// ================================
// ✅ FIX PRINCIPAL: emite register-peer assim que o peer abre
// ANTES de qualquer join-room
// ================================
peer.on('open', (id) => {
  console.log('🔄 Peer conectado com ID:', id);
  window.myPeerId = id;
  // Registra imediatamente no servidor
  socket.emit('register-peer', { peerId: id });
});

// Recebe chamada de outro peer
peer.on('call', (call) => {
  ...
  call.on('error', (err) => console.error('❌ Erro na chamada recebida:', err));
...
function addRemoteVideo(peerId, stream) {
  ...
  const tryPlay = () => {
    video.play()
      .then(() => console.log('✅ Play iniciado:', peerId))
...
function callPeer(peerId) {
  if (!peerId) return;
  if (peers[peerId]) { console.log('ℹ️ Já conectado com', peerId); return; }
  ...
  try {
    ...
    call.on('close', () => {
      console.log('🔇 Encerrado com:', peerId);
      removeRemoteVideo(peerId);
      delete peers[peerId];
    });

    call.on('error', (err) => {
      console.error('❌ Erro chamada para', peerId, err);
      delete peers[peerId];
    });
...
async function initializeMedia() {
  try {
    ...
    try {
      ...
      showNotification("Sem acesso a câmera/microfone: " + e.message, 'error');
      return;
    }
  }
...
async function shareScreen() {
  ...
  try {
    ...
    if (err.name !== 'NotAllowedError') showNotification("Erro ao compartilhar tela", 'error');
  }
}
...
function updateMicUI() {
  document.querySelectorAll('#micBtn').forEach(btn => {
    if (btn.querySelector('span')) {
      ...
      btn.innerHTML = audioEnabled ? '<i class="fas fa-microphone"></i>' : '<i class="fas fa-microphone-slash"></i>';
...
function deleteRoom(roomName) {
  if (!userRooms.has(roomName)) return;
...
function displayRooms(rooms) {
  roomsList.innerHTML = "";
  if (rooms.length === 0) {
    roomsList.innerHTML = '<div class="no-rooms">Nenhuma sala disponível</div>';
    return;
  }
  rooms.forEach(room => {
...
function addPanelToggles() {
  ...
  c.innerHTML = `
    <button onclick="toggleParticipantsPanel()" class="panel-toggle-btn" id="toggleParticipants" title="Participantes">
      ...
      <span class="badge" id="participantsBadge" style="display:none;"></span>
    </button>
...
function showHostPanel() {
  ...
  modal.innerHTML = `<div class="modal-content">
    <div class="modal-header">
      <i class="fas fa-crown"></i><h2>Controles do Host</h2>
...
function updateHostControlsList() {
  ...
  participants.forEach((p, id) => {
    const muted = mutedByHost.get(id);
    const blocked = screenBlockedByHost.get(id);
    html += `<div style="margin-bottom:1rem;padding:1rem;background:#f8f9fa;border-radius:8px;">
      <strong>${p.name}</strong>
      <div style="display:flex;gap:0.5rem;margin-top:0.5rem;">
        <button onclick="toggleMuteParticipant('${id}')" style="padding:0.5rem;border-radius:5px;border:none;cursor:pointer;background:${muted?'#dc3545':'#6c757d'};color:#fff;"><i class="fas ${muted?'fa-microphone-slash':'fa-microphone'}"></i></button>
        <button onclick="toggleScreenBlock('${id}')" style="padding:0.5rem;border-radius:5px;border:none;cursor:pointer;background:${blocked?'#dc3545':'#6c757d'};color:#fff;"><i class="fas fa-desktop"></i></button>
        <button onclick="kickParticipant('${id}')" style="padding:0.5rem;border-radius:5px;border:none;cursor:pointer;background:#dc3545;color:#fff;"><i class="fas fa-user-slash"></i></button>
      </div>
    </div>`;
  });
  list.innerHTML = html;
...
function playSound(audio) {
  if (soundsEnabled && audio?.play) audio.play().catch(() => {});
}
...
socket.on("room-error", (msg) => {
  ...
  if (msg.includes('existe') || msg.includes('criou')) {
    userCreatedRoom = false; userRooms.clear(); updateCreateRoomUI();
  }
});

// ================================
// ✅ room-joined: sem duplicata, sem setTimeout desnecessário
// ================================
socket.on("room-joined", async (roomData) => {
  ...
  // Chama participantes que já têm peerId
  if (roomData.participants?.length > 0) {
    console.log('👥 Participantes existentes:', roomData.participants);
    roomData.participants.forEach(p => {
      if (p.socketId !== socket.id) {
        participants.set(p.socketId, {
          name: p.name, peerId: p.peerId,
          speaking: false, handRaised: false, videoEnabled: false, audioEnabled: true
        });
        ...
        if (p.peerId) {
          ...
          setTimeout(() => callPeer(p.peerId), 1000);
        }
      }
    });
...
socket.on("user-connected", ({ socketId, name, peerId }) => {
  ...
  if (peerId) {
    // Aguarda um pouco para garantir que o stream local está pronto
    setTimeout(() => callPeer(peerId), 1500);
  } else {
    console.log('⏳ Usuário sem peerId ainda, aguardando peer-registered');
  }
});
...
socket.on("user-disconnected", (userId) => {
  ...
  if (p?.peerId) {
    removeRemoteVideo(p.peerId);
    if (peers[p.peerId]) { peers[p.peerId].close(); delete peers[p.peerId]; }
  }
...
socket.on("peer-registered", ({ socketId, peerId }) => {
  ...
  if (p) {
    ...
    participants.set(socketId, {
      ...
      peerId, speaking: false, handRaised: false, videoEnabled: false, audioEnabled: true
    });
    updateParticipantsList();
  }
...
socket.on("screen-blocked", ({ userId, blocked }) => {
  if (userId === socket.id) {
    ...
    if (blocked) showNotification("Bloqueado de compartilhar tela", 'warning');
  }
});
...
socket.on("user-speaking", ({ userId, speaking }) => {
  const p = participants.get(userId);
  if (p?.peerId) document.getElementById(`video-${p.peerId}`)?.classList.toggle('speaking', speaking);
});

socket.on("room-deleted", (roomName) => {
  if (currentRoom === roomName) { leaveRoom(); showNotification("Sala fechada pelo criador", 'warning'); }
});
...
function leaveRoom() {
  ...
  if (currentRoom && userRooms.has(currentRoom)) {
    userCreatedRoom = false; userRooms.delete(currentRoom); updateCreateRoomUI();
  }
